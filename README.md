...
Web UI では「テキストを貼り付け」「録音をアップロード」のどちらでも解析できます。テキスト入力はClaude APIへの解析のみのため数秒で完了し、音声アップロードはWhisperでの文字起こしを挟むため数十秒〜数分かかります（非同期ジョブ+ポーリングで処理するため、処理中に接続がタイムアウトすることはありません）。
