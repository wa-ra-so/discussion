...
```
商談音声ファイル または 文字起こし済みテキスト
    ↓
（音声の場合）Whisper による自動文字起こし
    ↓
...
Web UI では「テキストを貼り付け」「録音をアップロード」のどちらでも解析できます。テキスト入力はClaude APIへの解析のみのため数秒で完了し、音声アップロードはWhisperでの文字起こしを挟むため、録音の長さに応じて数分〜数十分かかります（非同期ジョブ+ポーリングで処理するため、処理中に接続がタイムアウトすることはありません）。

## 主な機能

- ✅ JSON スキーマ・Pydantic モデル（ディスカッション6セクション全対応）
- ✅ 音声ファイル（Whisper）またはテキストの、どちらの入力にも対応
...
画面は4つのタブで構成されています:
- **テキスト解析** — 文字起こし済みテキストの貼り付け、または録音ファイルのアップロードから選んで自動解析
...
# ※ CLIはテキスト入力のみ対応。音声ファイルの直接処理はWeb UIの「録音をアップロード」から行う
