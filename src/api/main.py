...
# ============================================================================
# 音声処理の最大待ち時間。
# 商談の録音は20分〜1時間程度になることを前提とする。CPUのみ（GPUなし）の
# 共有vCPUでの文字起こしは実時間より遅くなりうるため、正常なケースを
# 巻き込んで打ち切らないよう十分長く取る。それでもWhisperのffmpeg経由の
# 音声デコードがまれにハングして返ってこないケースがあるため、
# 「無限に pending のまま残る」事態だけは避けるための上限として設定する。
AUDIO_PROCESSING_TIMEOUT_SECONDS = 3600
...
async def process_audio(
    ...
    Whisper文字起こし + Claude解析には長い録音の場合数十分かかることがあるため、リクエストを
